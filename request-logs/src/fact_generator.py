...
class FactRequestLogGenerator:
    ...
    def generate_fact(
        self, timestamp: datetime = None, denormalized: bool = False
    ) -> Dict[str, Any]:
        ...
        time_to_first_byte = int(
            latency_ms * self.dim_mgr.fake.random.uniform(0.3, 0.6)
        )
        ...
        retry_count = (
            self.dim_mgr.fake.random.randint(1, 3)
            if is_error and self.dim_mgr.fake.random.random() < 0.3
            else 0
        )
    ...
    def _calculate_final_latency(self, base_latency: float, status) -> int:
        ...
        if status.is_success:
            return int(
                max(5, self.dim_mgr.fake.random.gauss(base_latency, base_latency * 0.4))
            )
        elif status.is_server_error:
            return int(
                max(
                    50,
                    self.dim_mgr.fake.random.gauss(base_latency * 4, base_latency * 2),
                )
            )
        else:
            return int(
                max(
                    5,
                    self.dim_mgr.fake.random.gauss(
                        base_latency * 0.6, base_latency * 0.3
                    ),
                )
            )
