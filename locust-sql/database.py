...
class DatabaseManager:
    """Manages DuckDB/Parquet operations for load test tracking."""

    def __init__(self, config):
        self.config = config
        self.db_file = "query_response_times.duckdb"
        self.run_id = None
            ...
            con.executemany(
                """INSERT INTO response_times
                   (run_id, timestamp, query_name, response_time_ms, status_code, success, error_message,
                    analyze_ms, optimize_ms, execute_ms, format_ms, thread_pool, plan)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                batch,
            )
            con.close()
    ...
    def record_response(
        ...
        if profile:
            ...
            analyze_ms = phases.get("analyze", {}).get("time_ms")
            optimize_ms = phases.get("optimize", {}).get("time_ms")
            execute_ms = phases.get("execute", {}).get("time_ms")
            format_ms = phases.get("format", {}).get("time_ms")
            thread_pool = profile.get("thread_pool")
            if profile.get("plan"):
                plan = json.dumps(profile["plan"])
